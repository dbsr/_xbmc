...
class Addon:
    ...
    def __init__(self, addon_id, mock_settings=None):
        '''        
        Args:
            addon_id (str): Your addon's id (eg. 'plugin.video.t0mm0.test').
            
        Kwargs:
            mock_settings (dict): used for retrieving settings during development.

        '''
        self.addon = xbmcaddon.Addon(id=addon_id)
        self.mock_settings = mock_settings
        _cur_ctx.append(self)
        if not os.path.isdir(self.get_profile()):
            os.mkdir(self.get_profile())

    def run(self, argv=None):
        if argv:
            self.url = argv[0]
            self.handle = int(argv[1])
            queries = self.parse_query(argv[2][1:])
            route = queries.get('route', '/')
            # if route exists and is registered send to dispatcher. Raise unless
            # default handler (catch all) is defined.
            rule = self.rules.get(route)
            if rule:
                self.dispatch(rule, queries)
        else:
            # raise
            pass

    def route(self, rule):
        '''Register decorated function as rule using decorators argument as key
        Example:
           @app.route('index')
           def index():
               pass

        :rule:  the url route used by the addons route dispatcher
    ...
    def dispatch(self, rule, queries=None):
        argnames = rule.func_code.co_varnames[:rule.func_code.co_argcount]
        if not argnames:
            return rule()
        kwargs = {}
        for k in argnames:
    ...
    def queries_for(self, target_rule, **kwargs):
        for route, rule in self.rules.items():
            if rule.__name__ == target_rule.lstrip('.'):
                kwargs['route'] = route
        return kwargs
    ...
    def get_name(self):    
        ...
        return unicode(self.addon.getAddonInfo('name').decode('utf8'))
    ...
    def get_setting(self, setting):
        ...
        '''
        if self.mock_settings:
            return self.mock_settings.get(setting)
    ...
    def log(self, msg, level=xbmc.LOGNOTICE):
        ...
        #                                                         'ignore')
        xbmc.log(msg.encode('ascii', 'ignore'), level)
    ...
    def show_search_dialog(self, heading, callback, do_last_search=True):
        last_search = ''
        if do_last_search:
            search_data = self.load_data('search_history')
            if search_data:
                last_search = search_data[-1]
            else:
                search_data = []
        kb = xbmc.Keyboard()
        ...
        if kb.isConfirmed():
            ...
            if do_last_search:
                ...
                self.save_data('search_history', search_data)
            callback(query)
